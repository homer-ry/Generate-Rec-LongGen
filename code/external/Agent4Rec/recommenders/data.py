...
try:
    # Optional dependency (pip install reckit). Some environments do not have it.
    from reckit import randint_choice  # type: ignore
except Exception:
    # Fallback to the local pure-numpy implementation shipped with this repo.
    try:
        # When running inside `recommenders/` as cwd (e.g. `python train_recommender.py`).
        from util.tool import randint_choice  # type: ignore
    except Exception:
        # When importing as a package from repo root (e.g. simulation code).
        from recommenders.util.tool import randint_choice  # type: ignore
import os
...
def helper_load(filename):
    ...
    with open(filename) as f:
        for line in f.readlines():
            ...
            # Keep empty users as explicit keys so downstream code can safely
            # access user_dict_list[user] without KeyError (e.g. simulation on
            # sparse valid/test splits).
...
def helper_load_train(filename):
    ...
    with open(filename) as f:
        for line in f.readlines():
            ...
            # Keep empty users as explicit keys (see helper_load).
            user_dict_list[user] = items
...
class Data:
    ...
    def __init__(self, args):
        # Allow selecting an alternative cf_data directory (e.g. "cf_data_min4")
        # while keeping the same dataset name for simulation alignment.
        cf_subdir = getattr(args, "cf_data_subdir", "cf_data")
        self.path = os.path.join(args.data_path, args.dataset, str(cf_subdir)) + os.sep
    ...
    def load_data(self):
        ...
        # Agent4Rec assumes ids are 0-based and (usually) contiguous. Some filtered datasets
        # may have missing ids (e.g., after removing low-rating interactions), where
        # `len(unique_items) != max_item_id+1`. Use max+1 as the safe cardinality.
        raw_users = sorted(
            set(self.train_user_list.keys())
            | set(self.valid_user_list.keys())
            | set(self.test_user_list.keys())
        )
        raw_items = sorted(set().union(*temp_lst))
        self.n_users = (max(raw_users) + 1) if raw_users else 0
        self.n_items = (max(raw_items) + 1) if raw_items else 0

        # Keep contiguous id lists for negative sampling/training utilities.
