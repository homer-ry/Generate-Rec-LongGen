...
class AbstractRS(nn.Module):
    ...
    def preperation_for_saving(self, args, special_args):
        ...
        # Windows-friendly fixed output dir: if saveID is literally "Saved", write to
        # weights/<dataset>/<model>/Saved so main.py can load it by default and
        # avoid overlong path names (WinError 206).
        if str(getattr(args, "saveID", "")).strip().lower() == "saved":
            self.saveID = "Saved"
