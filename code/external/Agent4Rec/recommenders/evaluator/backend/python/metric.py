...
def hr(rank, ground_truth):
    """Hit Ratio (a.k.a. Hit@K): 1 if any ground-truth item appears in top-K.

    This implementation returns a length-|rank| vector so it can be used as a
    per-K metric, consistent with other metrics in this file.
    """
    hits = np.array([1.0 if item in ground_truth else 0.0 for item in rank], dtype=np.float32)
    # HR@k is 1 if there is any hit within the first k items.
    return np.maximum.accumulate(hits).astype(np.float32)
