...
class UniEvaluator(AbstractEvaluator):
    ...
    * **Second**, this class and its evaluation metrics can automatically fit
    ...
    # Keep the signature aligned with the cpp backend so ProxyEvaluator can call both.
    ...
    def __init__(
        ...
        if metric is None:
            # Keep aligned with cpp backend so downstream code can index ret[5] as HR/HitRatio.
        ...
        self.user_pos_dump = dump_dict if dump_dict is not None else user_train_dict
    ...
    def evaluate(self, model, test_users=None):
        ...
        for batch_users in tqdm(test_users):
            if self.user_neg_test is not None:
                ...
                for idx, user in enumerate(batch_users):
                    train_items = self.user_pos_dump.get(user, [])
                    test_set = self.user_pos_test.get(user, set())
                    train_items = [x for x in train_items if x not in test_set]
        ...
        # Keep aligned with cpp backend: (ret_array, pretty_str)
