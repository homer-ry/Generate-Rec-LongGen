...
### Use All_Beauty Dataset

If you have downloaded Amazon `All_Beauty`, convert it to Agent4Rec format first:

```bash
python scripts/prepare_all_beauty_agent4rec.py --source_root ../../datasets/All_Beauty --target_dataset all-beauty
```

Then run a simulation:

```bash
python main.py --dataset all-beauty --modeltype Random --simulation_name BeautySmoke --n_avatars 20 --max_pages 3 --items_per_page 4
```

Notes:
- `all_personas_like_modify.csv` generated by the script includes a `timeline` field.
- The avatar prompt uses this timeline as chronological user history to better simulate preference drift.
- Prompt profiles are dataset-specific: legacy movie prompt is kept for `ml-1m` and previous datasets; `all-beauty` uses a dedicated beauty prompt.
- For `all-beauty`, items are displayed with `ID:` and the avatar responds using numeric IDs for robust parsing (titles can be very long).
- For a new dataset, `Random` and `Pop` can run directly; other recommenders require training weights on that dataset.
