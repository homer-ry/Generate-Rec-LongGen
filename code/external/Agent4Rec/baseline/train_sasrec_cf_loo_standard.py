...
def parse_args():
    p = argparse.ArgumentParser(description="Standard SASRec training (LOO) on Agent4Rec-format cf_data datasets")
...
def main():
    ...
    if not args.save_dir:
        args.save_dir = f"recommenders/weights/{args.dataset}/SASRec/Saved"
    if not args.metrics_out:
        args.metrics_out = f"baseline/results/sasrec_{args.dataset}_loo_standard_metrics.json"
    ...
    # Provide an args.txt so simulation can load dataset/batch_size defaults without
    # falling back to ml-1m args.
