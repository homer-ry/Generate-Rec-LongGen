...
def load_seq_data_cf(root: Path, dataset: str, cf_data_subdir: str = "cf_data") -> SeqData:
    ...
    for u in range(user_num):
        ...
        # Reserve 0 for padding: item ids in training are 1..item_num.
...
class GRU4RecTrainDataset(Dataset):
    """
    Train samples for next-item prediction.

    We produce (seq, pos) as right-padded arrays:
      seq[t] = item at time t
      pos[t] = next item after seq[t]
    """
    ...
    def __getitem__(self, index: int):
        ...
        if len(user_seq) >= 2:
            # Keep the most recent maxlen+1 items to build maxlen training pairs.
...
class GRU4Rec(nn.Module):
    ...
    def log2feats(self, log_seqs: torch.Tensor) -> torch.Tensor:
        """
        log_seqs: [B, L] right-padded (pad=0 at the tail).
        return feats: [B, L, H]
        """
    ...
    def score_all(self, log_seqs: torch.Tensor) -> torch.Tensor:
        feats = self.log2feats(log_seqs)  # [B, L, H]
        ...
        all_items = self.item_emb.weight[1:]  # [N, H]
    ...
    def score_items(self, log_seqs: torch.Tensor, item_indices: torch.Tensor) -> torch.Tensor:
        ...
        idx = (lengths - 1).view(-1, 1, 1).expand(-1, 1, feats.size(-1))
        final_feat = feats.gather(1, idx).squeeze(1)  # [B, H]
        item_embs = self.item_emb(item_indices)  # [B, K, H]
...
def parse_args():
    p = argparse.ArgumentParser(description="GRU4Rec training (LOO) on Agent4Rec-format cf_data datasets")
...
def train_one_epoch_full_ce(
    ...
    model.train()
    losses = []
    for _u, seq, pos in train_loader:
        seq = seq.to(device)
        pos = pos.to(device)
...
def main():
    ...
    if not args.save_dir:
        args.save_dir = f"recommenders/weights/{args.dataset}/GRU4Rec/Saved"
    if not args.metrics_out:
        args.metrics_out = f"baseline/results/gru4rec_{args.dataset}_loo_standard_metrics.json"

    data = load_seq_data_cf(root, dataset=args.dataset, cf_data_subdir=args.cf_data_subdir)
    train_ds = GRU4RecTrainDataset(data=data, maxlen=int(args.maxlen))
    train_loader = DataLoader(
        train_ds,
    ...
    # Provide an args.txt so simulation can load dataset defaults without falling back to ml-1m args.
