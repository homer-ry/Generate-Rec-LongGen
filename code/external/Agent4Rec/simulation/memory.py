...
class AvatarMemory(BaseMemory):
    ...
    def _safe_print(text: str) -> None:
        """Avoid Windows console encoding errors for unexpected Unicode chars."""
        try:
            print(text)
        except UnicodeEncodeError:
            try:
                safe_text = str(text).encode("utf-8", errors="replace").decode(
                    "utf-8", errors="replace"
                )
                print(safe_text)
            except Exception:
                print("[AvatarMemory] non-encodable output skipped")
    ...
    def get_completion(self, prompt, llm=None, temperature=0):
        ...
        while response == '':
            try:
                ...
                llm_model = llm if llm is not None else self.llm_model
    ...
    def _user_taste_reflection(self, last_k: int = 10) -> List[str]:
        ...
        result = self.get_completion(prompt=prompt_filled, llm=self.llm_model, temperature=0.2)
