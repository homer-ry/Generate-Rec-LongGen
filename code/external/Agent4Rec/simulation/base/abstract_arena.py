...
class abstract_arena:
    def __init__(self, args):
        ...
        self.modeltype = model_alias.get(str(args.modeltype).lower(), args.modeltype)
        self.args.modeltype = self.modeltype
    ...
    def load_saved_args(self, model_path):
        ...
        candidate_paths = [
            os.path.join(model_path, "args.txt"),
            os.path.join("recommenders", "weights", self.dataset, self.modeltype, "Saved", "args.txt"),
            os.path.join("recommenders", "weights", "ml-1m", self.modeltype, "Saved", "args.txt"),
            os.path.join("recommenders", "weights", "default_args.txt"),
        ]
        loaded_path = None
        for args_path in candidate_paths:
            if os.path.exists(args_path):
                # PowerShell may write UTF-8 files with BOM; use utf-8-sig to be robust.
                with open(args_path, "r", encoding="utf-8-sig") as f:
                    self.saved_args.__dict__ = json.load(f)
                loaded_path = args_path
                break
        if loaded_path is None:
            raise FileNotFoundError(
                f"No args.txt found for model {self.modeltype}. Checked: {candidate_paths}"
            )
        print(f"Loaded recommender args from: {loaded_path}")
        ...
        # Let model implementations know which weights subdir is being used.
        self.saved_args.model_path = getattr(self.args, "model_path", "Saved")
    ...
    def prepare_dir(self):
        ...
        run_args = dict(vars(self.args))
        if run_args.get("openai_api_key"):
            run_args["openai_api_key"] = "***redacted***"
        run_config = {
        ...
        with open(self.storage_base_path + "/run_config.json", "w", encoding="utf-8") as f:
    ...
    def load_recommender(self):
        ...
        if self.args.modeltype not in no_ckpt_models:
